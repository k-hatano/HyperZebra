...
public class OCardBase extends OObject {
	...
	public Boolean showPict = true;
	public MyLabel label;
	public OPicture picture;
	public String bitmapName;

	public boolean changed;

	// ボタン、フィールド情報
	public ArrayList<OObject> partsList;
	public ArrayList<OButton> btnList;
	public ArrayList<OField> fldList;
	...
	public OBackground getBg() {
		OBackground bg = null;
		if (this.objectType.equals("background"))
			bg = (OBackground) this;
			...
			if (cd.bg != null) {
				...
				bg = cd.stack.GetBackgroundbyId(cd.bgid);
			}
		}
		return bg;
	}
	...
	public int GetNumberofParts(OObject obj) {
		for (int number = 0; number < partsList.size(); number++) {
			if (partsList.get(number).id == obj.id && partsList.get(number).objectType.equals(obj.objectType)) {
				return number + 1;
			}
		}
		return 0;
