...
public class OButton extends OObject {
	...
	public MyButton btn = null;
	MyCheck chkbox = null;
	public MyRadio radio = null;
	public MyPopup popup = null;
	public BufferedImage imageForScale; // アイコン拡大縮小用データ;
	...
	public int group = 0;
	public Boolean check_hilite = false;// 他のカードからの参照用
	private boolean autoHilite = true;// 作成時のみ参照
	private boolean hilite = false;// 作成時のみ参照
	public int icon = 0;
	public URI iconURI;
	// number (カードの情報から求める)
	public Boolean sharedHilite = false;
	public Boolean showName = true;
	public int style = 0;// 0標準 1透明 2不透明 3長方形 4シャドウ 5丸みのある長方形 6省略時設定 7楕円 8ポップアップ 9チェックボックス 10ラジオ
	public int textAlign = 1;// 0左 1中 2右
	...
	public boolean scaleIcon = false;
	...
	public static ButtonGroup[] btnGroup = new ButtonGroup[16];
		...
		if (btn != null) {
			...
			if (icon > 0) {
				if (in == cache_id) {
					/*
					 * if((autoHilite ||
					 * hilite==btn.getModel().isArmed())&&!showName&&style>=1&&style<=3){
					 * this.iconImage = cache_bi; }else
					 */ {
						if (scaleIcon || blendMode != 0)
							imageForScale = cache_bi;
					...
					if (path != null) {
						// String path =
						// (((OCardBase)parent).stack.file.getParent()+File.separatorChar+fileName);
						...
						if (file.exists()) {
							...
							// btn.setIcon(new
							// ImageIcon(card.stack.file.getParent()+"/resource/icon"+icon+".png"));
						} else {
							try {
				...
				if (!PCARD.lockedScreen && noupdate == false) {
					/*
					 * if(cache_bi!=null){ //もうちょっとちゃんとしないといけないが高速化 Graphics paneg =
					 * PCARD.pc.mainPane.getGraphics();
					 * paneg.drawImage(cache_bi,left,top,left+width,top+height,0,0,width,height,
					 * PCARD.pc.mainPane); }else
					 */ {
						PCARD.pc.mainPane.paintImmediately(left, top, width, height);
				...
				if (path != null) {
					...
					if (!file.exists()) {
						try {
							BufferedImage pict = ImageIO
									.read(new File(card.stack.file.getParent() + "/resource/pict" + name + "_img.png"));
				...
				// btn.setIcon(new
				// ImageIcon(card.stack.file.getParent()+"/resource/pict"+name+".png"));
				btn.setText("");
				if (!PCARD.lockedScreen && noupdate == false) {
					PCARD.pc.mainPane.paintImmediately(left, top, width, height);
					if (TTalk.wait > 0 && PCARD.pc.stack.createdByVersion.charAt(0) == '2') {
		...
		if (btn != null) {
			if (iconURI != null) {
				try {
					...
					if (scaleIcon || blendMode != 0) {
						imageForScale = new BufferedImage(img.getWidth(btn), img.getHeight(btn),
	...
	/*
	 * static NullPaintManager nullPaintManager = new NullPaintManager(); static
	 * class NullPaintManager extends RepaintManager{
	 * 
	...
	public void setTopLeft(int h, int v) {
		...
		if (getComponent() != null) {
			/*
			 * RepaintManager rm = null; if(!PCARD.lockedScreen) { rm =
			 * RepaintManager.currentManager(getComponent());
			 * RepaintManager.setCurrentManager(nullPaintManager); }
			 */
			getComponent().setBounds(left, top, width, height);
			/*
			 * if(!PCARD.lockedScreen) { RepaintManager.setCurrentManager(rm); }
			 */
	...
	public boolean getAutoHilite() {
		/*
		 * if(btn != null){ return btn.autoHilite; }
		 */
		return autoHilite;
	}
			...
			try {
				...
				if (eventType == XMLStreamReader.START_ELEMENT) {
					...
					if (elm.equals("id")) {
						...
						reader.next();
					} else if (elm.equals("reserved5")) {
						this.reserved5 = reader.getElementText();
					} else if (elm.equals("reserved4")) {
						this.reserved4 = reader.getElementText();
					} else if (elm.equals("reserved3")) {
						this.reserved3 = reader.getElementText();
					} else if (elm.equals("reserved2")) {
						this.reserved2 = reader.getElementText();
					} else if (elm.equals("reserved1")) {
						this.reserved1 = reader.getElementText();
					} else if (elm.equals("enabled")) {
						...
						if (0 == tmpstr.compareTo("shadow"))
							this.style = 4;
						if (0 == tmpstr.compareTo("roundrect"))
							this.style = 5;
						if (0 == tmpstr.compareTo("default"))
							this.style = 6;
						if (0 == tmpstr.compareTo("oval"))
							this.style = 7;
						if (0 == tmpstr.compareTo("popup"))
							this.style = 8;
						if (0 == tmpstr.compareTo("checkbox"))
							this.style = 9;
						if (0 == tmpstr.compareTo("radio"))
							this.style = 10;
					} else if (elm.equals("showName")) {
						this.showName = XMLRead.bool(reader);
						reader.next();
					} else if (elm.equals("highlight")) {
						this.hilite = XMLRead.bool(reader);
						reader.next();
					} else if (elm.equals("autoHighlight")) {
						this.autoHilite = XMLRead.bool(reader);
						reader.next();
					} else if (elm.equals("sharedHighlight")) {
						this.sharedHilite = XMLRead.bool(reader);
						reader.next();
					} else if (elm.equals("family")) {
						this.group = Integer.valueOf(reader.getElementText());
					} else if (elm.equals("titleWidth")) {
						this.titleWidth = Integer.valueOf(reader.getElementText());
						...
						this.icon = Integer.valueOf(reader.getElementText());
					} else if (elm.equals("iconURI")) {
						this.iconURI = new URI(reader.getElementText());
						...
						if (tmpstr.contains("group"))
							this.textStyle += 128;
					} else if (elm.equals("reserved25")) {
						reserved25 = reader.getElementText();
					} else if (elm.equals("scaleIcon") || elm.equals("scalingIcon")) {
						this.scaleIcon = XMLRead.bool(reader);
						reader.next();
					} else if (elm.equals("blendingMode")) {
						String tmpstr = reader.getElementText();
						if (tmpstr.equals("copy"))
							this.blendMode = 0;
						if (tmpstr.equals("blend"))
							this.blendMode = 1;
						if (tmpstr.equals("add"))
							this.blendMode = 2;
						if (tmpstr.equals("subtract"))
							this.blendMode = 3;
						if (tmpstr.equals("multiply"))
							this.blendMode = 4;
						if (tmpstr.equals("screen"))
							this.blendMode = 5;
						if (tmpstr.equals("darken"))
							this.blendMode = 6;
						if (tmpstr.equals("lighten"))
							this.blendMode = 7;
						if (tmpstr.equals("difference"))
							this.blendMode = 8;
						if (tmpstr.equals("hue"))
							this.blendMode = 9;
						if (tmpstr.equals("color"))
							this.blendMode = 10;
						if (tmpstr.equals("saturation"))
							this.blendMode = 11;
						if (tmpstr.equals("luminosity"))
							this.blendMode = 12;
					} else if (elm.equals("blendingLevel")) {
						this.blendLevel = Integer.valueOf(reader.getElementText());
	...
	public void writeXML(XMLStreamWriter writer) throws XMLStreamException {
		...
		case 4:
			...
			break;
		case 5:
			writer.writeCharacters("roundrect");
			break;
		case 6:
			writer.writeCharacters("default");
			break;
		case 7:
			writer.writeCharacters("oval");
			break;
		case 8:
			writer.writeCharacters("popup");
			break;
		case 9:
			writer.writeCharacters("checkbox");
			break;
		case 10:
			writer.writeCharacters("radio");
			break;
		}
		writer.writeEndElement();
		writer.writeCharacters("\n\t\t\t");
		...
		if (parent.getClass() == OBackground.class) {
			...
			/*
			 * writer.writeStartElement("reserved25"); writer.writeCharacters(" 0 ");
		...
		switch (blendMode) {
		case 0:
			writer.writeCharacters("copy");
			break;
		case 1:
			writer.writeCharacters("blend");
			break;
		case 2:
			writer.writeCharacters("add");
			break;
		case 3:
			writer.writeCharacters("subtract");
			break;
		case 4:
			writer.writeCharacters("multiply");
			break;
		case 5:
			writer.writeCharacters("screen");
			break;
		case 6:
			writer.writeCharacters("darken");
			break;
		case 7:
			writer.writeCharacters("lighten");
			break;
		case 8:
			writer.writeCharacters("difference");
			break;
		case 9:
			writer.writeCharacters("hue");
			break;
		case 10:
			writer.writeCharacters("color");
			break;
		case 11:
			writer.writeCharacters("saturation");
			break;
		case 12:
			writer.writeCharacters("luminosity");
			break;
		}
		writer.writeCharacters(" ");
		writer.writeEndElement();
	...
	public static void buildOButton(OButton obtn) {
		// swingのボタンを追加
		...
		if (obtn.style == 1 || obtn.style == 2 || obtn.style == 3 || obtn.style == 4)
			obtn.btn = new RectButton(obtn, obtn.name);
		if (obtn.style == 5)
			obtn.btn = new RoundedCornerButton(obtn, obtn.name);
		if (obtn.style == 7)
			obtn.btn = new RoundButton(obtn, obtn.name);
		...
		if (obtn.style <= 7) {
			...
			/*
			 * if(obtn.icon > 0) { File file=new
			 * File(obtn.card.stack.file.getParent()+"/resource/icon"+obtn.icon+".png");
			 * if(file.exists()) { obtn.btn.setIcon(new
			 * ImageIcon(obtn.card.stack.file.getParent()+"/resource/icon"+obtn.icon+".png")
			 * ); } else { file=new
			 * File(obtn.card.stack.file.getParent()+"/resource/icon"+obtn.icon+".jpeg");
			 * if(file.exists()) { obtn.btn.setIcon(new
			 * ImageIcon(obtn.card.stack.file.getParent()+"/resource/icon"+obtn.icon+".jpeg"
			 * )); } else { try { BufferedImage pict = ImageIO.read(new
			 * File(obtn.card.stack.file.getParent()+"/resource/icon"+obtn.icon+"_img.png"))
			 * ; File file2=new
			 * File(obtn.card.stack.file.getParent()+"/resource/icon"+obtn.icon+"_mask.png")
			 * ; if(file2.exists()) { BufferedImage mask = ImageIO.read(file); pict =
			 * OCard.makeAlphaImage(pict, mask); File ofile=new
			 * File(obtn.card.stack.file.getParent()+"/resource/icon"+obtn.icon+".png");
			 * ImageIO.write(pict,"png",ofile); } }catch (Exception err) {
			 * //err.printStackTrace(); } } }
			 * obtn.btn.setHorizontalTextPosition(JButton.CENTER);
			 * obtn.btn.setVerticalTextPosition(JButton.BOTTOM); } if(obtn.icon == -1) {
			 * File file=new
			 * File(obtn.card.stack.file.getParent()+"/resource/pict"+obtn.name+".png");
			 * if(!file.exists()) { file=new
			 * File(obtn.card.stack.file.getParent()+"/resource/pict"+obtn.name+".jpeg"); }
			 * if(!file.exists()) { try { BufferedImage pict = ImageIO.read(new
			 * File(obtn.card.stack.file.getParent()+"/resource/pict"+obtn.name+"_img.png"))
			 * ; File file2=new
			 * File(obtn.card.stack.file.getParent()+"/resource/pict"+obtn.name+"_mask.png")
			 * ; if(file2.exists()) { BufferedImage mask = ImageIO.read(file); pict =
			 * OCard.makeAlphaImage(pict, mask); File ofile=new
			 * File(obtn.card.stack.file.getParent()+"/resource/pict"+obtn.name+".png");
			 * ImageIO.write(pict,"png",ofile); } }catch (Exception err) {
			 * //err.printStackTrace(); } } obtn.btn.setIcon(new
			 * ImageIcon(obtn.card.stack.file.getParent()+"/resource/pict"+obtn.name+".png")
			 * ); obtn.btn.setText(""); }
			 */
			switch (obtn.textAlign) {
			case 0:
				obtn.btn.setHorizontalAlignment(JButton.LEFT);
				break;
			case 1:
				obtn.btn.setHorizontalAlignment(JButton.CENTER);
				break;
			case 2:
				obtn.btn.setHorizontalAlignment(JButton.RIGHT);
				break;
			}
			...
			try {
				obtn.popup.setSelectedIndex(obtn.selectedLine - 1);
			} catch (Exception e) {
				System.out.println("err: popup.setSelectedIndex(selectedLine-1)");
			}
			obtn.popup.setFont(new Font(obtn.textFont, obtn.textStyle, obtn.textSize));
			if (obtn.showName)
				obtn.popup.setToolTipText(obtn.name);
	...
	public static btnOutlineListen btnOutlineListen = new btnOutlineListen();
