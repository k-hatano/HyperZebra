...
public class OCard extends OCardBase {
	public OBackground bg = null;
	public int bgid;

	ArrayList<OBgButtonData> bgbtnList;
	public ArrayList<OBgFieldData> bgfldList;
		...
		while (reader.hasNext()) {
			try {
				...
				if (eventType == XMLStreamReader.START_ELEMENT) {
					...
					if (elm.equals("id")) {
					...
					} else if (elm.equals("filler1")) {
						filler1 = reader.getElementText();
					} // markedかな？
					else if (elm.equals("bitmap")) {
						bitmapName = reader.getElementText();
						...
						reader.next();
					} else if (elm.equals("showPict")) {
						showPict = XMLRead.bool(reader);
						reader.next();
					} else if (elm.equals("dontSearch")) {
						dontSearch = XMLRead.bool(reader);
						reader.next();
						...
						while (reader.hasNext()) {
							...
							if (eventType2 == XMLStreamReader.START_ELEMENT) {
								...
								if (elm2.equals("id")) {
									partid = Integer.valueOf(reader.getElementText());
									...
									if (type.equals("button")) {
										...
										System.out.println("Part_Type: " + type);
									break;
								} else {
						...
						while (reader.hasNext()) {
							...
							if (eventType2 == XMLStreamReader.START_ELEMENT) {
								String elm2 = reader.getLocalName();
								if (elm2.equals("layer")) {
									layer = reader.getElementText();
								} else if (elm2.equals("id")) {
									partid = Integer.valueOf(reader.getElementText());
									...
									bgbtn.check_hilite = XMLRead.bool(reader);
									reader.next();
									...
									while (reader.hasNext()) {
										...
										if (eventType3 == XMLStreamReader.START_ELEMENT) {
											...
											if (elm3.equals("offset")) {
												/* offset = Integer.valueOf(reader.getElementText()); */ } else if (elm3
														.equals("id")) {
		...
		for (int i = 0; i < partsList.size(); i++) {
			...
			if (part.getText().length() > 0) {
				...
				writer.writeCharacters("\n\t\t\t");
				{
					...
					if (part.objectType.equals("field") && ((OField) part).styleList != null) {
	...
	public boolean readCardBlock(DataInputStream dis, int blockSize) {
		...
		for (int i = 0; i < numofParts; i++) {
			...
			if (dataLen < 30) {
				// System.out.println("!");
				/*
				 * if(dataLen>=0){ dataLen = (dataLen<<8) + HCData.readCode(dis, 1); }
				 */
			}
		...
		for (int i = 0; i < numofContents; i++) {
			...
			int pid;
			/*
			 * {//アライメント調整 pid = HCData.readCode(dis, 1); while(pid<=0 || (pid==255 &&
			 * i<255) || (pid==254 && i<255)){ pid = (pid<<8) + HCData.readCode(dis, 1); } }
			 */
			{
			...
			if (isStyledText < 128) {
				...
				for (int j = 0; j < formattingLength / 4; j++) {
					...
					if (pid > 32768) {
						...
						for (int k = 0; k < partsList.size(); k++) {
							if (partsList.get(k).id == inpid) {
								if (partsList.get(k).objectType.equals("field")) {
									...
									if (fld.styleList == null)
										fld.styleList = new ArrayList<styleClass>();
						...
						// bg part
						if (bgfld.styleList == null)
							bgfld.styleList = new ArrayList<styleClass>();
		...
		/*
		 * if(offset%2==0){ String paddingx = HCData.readStr(dis, 1); offset++;
	...
	private static OCard buildOCard(OCard cd) {
		...
		try {
			if (file.exists()) {
				...
				cd.label = MyLabel
						.getMyCdLabel(cd/* new ImageIcon(stack.file.getParent()+"/resource/cdp"+this.id+".png") */);
				cd.label.setBounds(0, 0, cd.stack.width, cd.stack.height);
				PCARD.pc.mainPane.add(cd.label);
			} else {
				...
				if (file.exists()) {
					...
					cd.label = MyLabel.getMyCdLabel(
							cd/* new ImageIcon(stack.file.getParent()+"/resource/cdp"+this.id+".jpeg") */);
					cd.label.setBounds(0, 0, cd.stack.width, cd.stack.height);
					PCARD.pc.mainPane.add(cd.label);
				}
			}
	...
	public static void reloadCurrentCard() {
		...
		try {
			...
			PCARD.pc.stack.curCard.bg = OBackground.getOBackground(PCARD.pc.stack, PCARD.pc.stack.curCard,
					PCARD.pc.stack.curCard.bgid, false);
