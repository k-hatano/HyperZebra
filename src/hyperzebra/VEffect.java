...
public class VEffect {
	...
	public static void visualEffect(int effect, int toEffect, int speed) {
		...
		if (effect == 0) {
			...
			if (effect == 1) { // barn door open
				while (now < end) {
					...
					paneg.drawImage(off, width / 2 - v, 0, width / 2 + v, height, width / 2 - v, 0, width / 2 + v,
							height, PCARDFrame.pc.stack.pcard);
				...
				while (now < end) {
					...
					paneg.drawImage(off, width - v, 0, width, height, width - v, 0, width, height,
							PCARDFrame.pc.stack.pcard);
					now = new Date().getTime();
				}
				...
				while (now < end) {
					...
					for (int i = 0; i < (height + 31) / 32; i++) {
						paneg.drawImage(off, 0, 32 * i, width, 32 * i + v, 0, 32 * i, width, 32 * i + v,
								PCARDFrame.pc.stack.pcard);
					}
					now = new Date().getTime();
				}
				...
				while (now < end) {
					...
					for (int i = 0; i < (height + 63) / 64; i++) {
						for (int j = 0; j < (width + 63) / 64; j++) {
							paneg.drawImage(off, j * 64, 64 * i, j * 64 + 32, 64 * i + v, j * 64, 64 * i, j * 64 + 32,
									64 * i + v, PCARDFrame.pc.stack.pcard);
						}
						for (int j = 0; j < (width + 63) / 64; j++) {
							paneg.drawImage(off, j * 64 + 32, 64 * i - 32, j * 64 + 64, 64 * i - 32 + v, j * 64 + 32,
									64 * i - 32, j * 64 + 64, 64 * i - 32 + v, PCARDFrame.pc.stack.pcard);
						}
					}
					now = new Date().getTime();
				}
				...
				while (now < end) {
					...
					paneg.drawImage(off, width / 2 - h, height / 2 - v, width / 2 + h, height / 2 + v, width / 2 - h,
							height / 2 - v, width / 2 + h, height / 2 + v, PCARDFrame.pc.stack.pcard);
					now = new Date().getTime();
				}
				...
				while (now < end) {
					...
					paneg.drawImage(off, 0, height - v, width, height, 0, height - v, width, height,
							PCARDFrame.pc.stack.pcard);
					paneg.drawImage(off, 0, v, h, height - v, 0, v, h, height - v, PCARDFrame.pc.stack.pcard);
					paneg.drawImage(off, width - h, v, width, height - v, width - h, v, width, height - v,
							PCARDFrame.pc.stack.pcard);
					now = new Date().getTime();
				}
				...
				while (now < end) {
					...
					paneg.drawImage(oldoff, 0, height / 2 - v, width, height / 2 + v, 0, 0, width, height,
							PCARDFrame.pc.stack.pcard);
					paneg.drawImage(off, 0, 0, width, height / 2 - v, 0, 0, width, height / 2 - v,
							PCARDFrame.pc.stack.pcard);
					paneg.drawImage(off, 0, height / 2 + v, width, height, 0, height / 2 + v, width, height,
							PCARDFrame.pc.stack.pcard);
					now = new Date().getTime();
				}
				...
				while (now < end) {
					int v = (int) (height / 2 * ((double) (now - start) / (end - start)));
					paneg.drawImage(off, 0, height / 2 - v, width, height / 2 + v, 0, 0, width, height,
							PCARDFrame.pc.stack.pcard);
					now = new Date().getTime();
				}
		...
		// 移動後のカードの表示
		PCARD.lockedScreen = false;
		// PCARD.pc.mainPane.paintImmediately(0, 0, PCARD.pc.stack.width,
		// PCARD.pc.stack.height);
