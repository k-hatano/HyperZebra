...
public class XMLRead {
	...
	public static boolean readToc(String xmlfile, OStack inStack) {
		...
		if (inStack.firstCard != 0 && inStack.GetCardbyId(inStack.firstCard) == null) {
			...
			if (result == false) {
				new GDialog(inStack.pcard, PCARDFrame.pc.intl.getDialogText("Error occured at reading XML file."), null,
						"OK", null, null);
				result = true;// 途中までしか読めなくても強制的に開いてみる
			} else if (completeFlag == false) {
				new GDialog(inStack.pcard, PCARDFrame.pc.intl.getDialogText("XML end tag is not found."), null, "OK",
						null, null);
				result = true;// 途中までしか読めなくても強制的に開いてみる
			}
		}
	...
	public static boolean bool(XMLStreamReader reader) throws XMLStreamException {
		while (true) {
			int eventType = reader.next();
			if (eventType == XMLStreamConstants.START_ELEMENT) {
				if (reader.getLocalName().equals("true"))
					return true;
				else if (reader.getLocalName().equals("false"))
					return false;
				else
					throw new XMLStreamException("not bool.");
			}
			if (eventType == XMLStreamConstants.END_ELEMENT) {
	...
	/*
	 * StackSmith日本語文字化け対応 final static char[] macroman = {
	 ...
	 * 0x00AF,0x02D8,0x02D9,0x02DA,0x00B8,0x02DD,0x02DB,0x02C7,// F8- FF }; static
	 * ByteBuffer toAscii(String src){ ByteBuffer bb =
	 * ByteBuffer.allocate(src.length());
	 * 
	 * for(int i=0; i<src.length(); i++){ char c = src.charAt(i); if(c>=0&&c<128){
	 * bb.put((byte)c); } else { int j=128; for(; j<256; j++){ if(c ==
	 * macroman[j-128]){ byte roman = (byte)j;
	 * //System.out.println("cnv:("+j+") roman="+Integer.valueOf(roman)+" c="
	 * +Integer.valueOf(c)); bb.put(roman); break; } } if(j>=256) bb.put((byte)c); }
	 * } return bb; }
	 * 
	 * static String fromSJIS(ByteBuffer bb){ bb.flip(); Charset cs =
	 * Charset.forName("Shift_JIS"); CharsetDecoder decoder = cs.newDecoder();
	 * 
	 * CharBuffer decBuf = null; try { decBuf = decoder.decode(bb); } catch
	 * (CharacterCodingException e) { //e.printStackTrace();
	 * //System.out.println(e.getMessage()); return bb.asCharBuffer().toString(); }
	 * 
