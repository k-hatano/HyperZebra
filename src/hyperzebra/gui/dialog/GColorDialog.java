...
public class GColorDialog extends JDialog implements ActionListener {
	...
	private GColorDialog(Frame frame, Color color, boolean useCapture) {
		...
		/*
		 * if(point!=null){ setBounds(point.x, point.y,
		 * chooser.getPreferredSize().width, chooser.getPreferredSize().height+40);
		 * }else if(frame!=null){ setBounds(frame.getX()+frame.getWidth()/2-300/2,
		 * frame.getY()+frame.getHeight()/2-200/2, chooser.getPreferredSize().width,
		 * chooser.getPreferredSize().height+40); }else
		 */ {
			PointerInfo info = MouseInfo.getPointerInfo();
			setBounds(info.getLocation().x - 180, info.getLocation().y - 150, chooser.getPreferredSize().width,
					chooser.getPreferredSize().height + 40);
		}
...
class CaptureThread extends Thread {
	...
	public void run() {
		...
		while (!clicked /* && owner.isVisible() */) {
			PointerInfo info = MouseInfo.getPointerInfo();
			if (lastinfo != null && info.getLocation().x == lastinfo.getLocation().x
