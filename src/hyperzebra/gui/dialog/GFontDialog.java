...
public class GFontDialog extends JDialog implements ActionListener, MouseListener {
	...
	public static int selectedAlign;
	public static MyTextArea sampleArea;
	PCARDFrame owner;

	public GFontDialog(PCARDFrame owner, String defaultFont, int defaultSize, int defaultStyle, int defaultAlign) {
		super(owner, true);
		this.owner = owner;
		...
		// フォントリスト
		final String fontFamilyNames[] = GraphicsEnvironment.getLocalGraphicsEnvironment()
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (name.equals("Bold")) {
			selectedStyle = (selectedStyle & (~1)) | (((JCheckBox) e.getSource()).isSelected() ? 1 : 0);
		} else if (name.equals("Italic")) {
			selectedStyle = (selectedStyle & (~2)) | (((JCheckBox) e.getSource()).isSelected() ? 2 : 0);
		} else if (name.equals("Underline")) {
			selectedStyle = (selectedStyle & (~4)) | (((JCheckBox) e.getSource()).isSelected() ? 4 : 0);
		} else if (name.equals("Outline")) {
			selectedStyle = (selectedStyle & (~8)) | (((JCheckBox) e.getSource()).isSelected() ? 8 : 0);
		} else if (name.equals("Shadow")) {
			selectedStyle = (selectedStyle & (~16)) | (((JCheckBox) e.getSource()).isSelected() ? 16 : 0);
		} else if (name.equals("Condensed")) {
			selectedStyle = (selectedStyle & (~32)) | (((JCheckBox) e.getSource()).isSelected() ? 32 : 0);
		} else if (name.equals("Extend")) {
			selectedStyle = (selectedStyle & (~64)) | (((JCheckBox) e.getSource()).isSelected() ? 64 : 0);
	...
	public void mousePressed(MouseEvent e) {
		...
		{
			int line = (e.getY() /* + area.pr_scrl.getVerticalScrollBar().getValue() */ + area.getLineHeight() - 1)
					/ area.getLineHeight();
