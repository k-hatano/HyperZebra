...
import hyperzebra.gui.TextEditor;
import hyperzebra.gui.button.AuthColorButton;
...
public class AuthDialog extends JDialog {
	...
	*/
	...
	public static void openAuthDialog(Frame owner, String type, OObject obj) {
		...
		if (type.equals("button")) {
			...
			// パネルを追加する
			{
				...
				String typeStr = "";
				if (obtn.parent.objectType.equals("card"))
					typeStr = PCARD.pc.intl.getDialogText("Card button ");
				else
					typeStr = PCARD.pc.intl.getDialogText("Background button ");
				...
				infoPanel.add(label);
				label = new JLabel(typeStr + PCARD.pc.intl.getDialogText("Number: ")
						+ ((OCardBase) obtn.parent).GetNumberof(obtn));
				label.setPreferredSize(new Dimension(w, label.getPreferredSize().height));
				infoPanel.add(label);
				label = new JLabel(typeStr + PCARD.pc.intl.getDialogText("Part Number: ")
						+ ((OCardBase) obtn.parent).GetNumberofParts(obtn));
				label.setPreferredSize(new Dimension(w, label.getPreferredSize().height));
				infoPanel.add(label);
			...
			// パネルを追加する
			{
				...
				popup = new JComboBox(new String[] { PCARD.pc.intl.getDialogText("Standard"),
						PCARD.pc.intl.getDialogText("Transparent"), PCARD.pc.intl.getDialogText("Opaque"),
						PCARD.pc.intl.getDialogText("Rectangle"), PCARD.pc.intl.getDialogText("Shadow"),
						PCARD.pc.intl.getDialogText("RoundRect"), PCARD.pc.intl.getDialogText("Default"),
						PCARD.pc.intl.getDialogText("Oval"), PCARD.pc.intl.getDialogText("Popup"),
						PCARD.pc.intl.getDialogText("CheckBox"), PCARD.pc.intl.getDialogText("Radio") });
			...
			// パネルを追加する
			{
				...
				famiPanel.add(label);
				popup = new JComboBox(new String[] { PCARD.pc.intl.getDialogText("None"), "1", "2", "3", "4", "5", "6",
			...
			// パネルを追加する
			{
				 ...
				 * jbtn.addActionListener(btnlistener); jbtn.setEnabled(false);
				 * buttonPanel.add(jbtn); jbtn = new
				 * JButton(PCARD.pc.intl.getDialogText("Effect…"));
				 * jbtn.addActionListener(btnlistener); jbtn.setEnabled(false);
				 * buttonPanel.add(jbtn);
				 */
		...
		if (type.equals("field")) {
			...
			// パネルを追加する
			{
				...
				String typeStr = "";
				if (ofld.parent.objectType.equals("card"))
					typeStr = PCARD.pc.intl.getDialogText("Card field ");
				else
					typeStr = PCARD.pc.intl.getDialogText("Background field ");
				label = new JLabel(typeStr + PCARD.pc.intl.getDialogText("ID: ") + ofld.id);
				label.setPreferredSize(new Dimension(w, label.getPreferredSize().height));
				infoPanel.add(label);
				label = new JLabel(typeStr + PCARD.pc.intl.getDialogText("Number: ")
						+ ((OCardBase) ofld.parent).GetNumberof(ofld));
				label.setPreferredSize(new Dimension(w, label.getPreferredSize().height));
				infoPanel.add(label);
				label = new JLabel(typeStr + PCARD.pc.intl.getDialogText("Part Number: ")
						+ ((OCardBase) ofld.parent).GetNumberofParts(ofld));
				label.setPreferredSize(new Dimension(w, label.getPreferredSize().height));
				infoPanel.add(label);
	...
	static class AuthButtonListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (cmd.equals("style")) {
				...
				new GFontDialog(PCARD.pc, ((OButton) object).textFont, ((OButton) object).textSize,
						((OButton) object).textStyle, ((OButton) object).textAlign);
	...
	static class AuthFieldListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (cmd.equals("style")) {
				...
				new GFontDialog(PCARD.pc, ((OField) object).textFont, ((OField) object).textSize,
						((OField) object).textStyle, ((OField) object).textAlign);
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (cmd.equals("Script…")) {
				...
				for (int i = 0; i < PCARD.pc.stack.cardIdList.size(); i++) {
					...
					str += "card(" + i + ") id:" + id + " name:" + card.name + " btns:" + card.btnList.size() + " flds:"
							+ card.fldList.size() + "\n";
				}
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand().equals("OK")) {
				...
				if (width <= 32 || height <= 32 || width * height >= 4000 * 3000) {
					new GDialog(PCARD.pc, PCARD.pc.intl.getDialogText("Illegal size."), null, "OK", null, null);
				} else {
