...
public class TextEditor extends JFrame {
	...
	/*
	 * static boolean saveAlert(TextArea area){ if(area == null || area.saved) {
	 * return true; } saveScript(area); return true; }
	 * 
	 * static void saveScript(TextArea area){ String allStr = ""; try { allStr =
	 * area.getDocument().getText(0, area.getDocument().getLength()); } catch
	 * (BadLocationException e) { e.printStackTrace(); }
	 * 
	 * OObject obj = area.object;
	 * 
	 * if(obj != null){ obj.text = allStr; area.savedScript(); } else {
...
class TextArea extends JTextArea {
	...
	void changedScript() {
		saved = false;
		// TextEditor.TextEditor.getRootPane().putClientProperty("windowModified",
		// Boolean.TRUE);
		setCanUndo();
	}
...
class TEMenu {
	...
	public TEMenu(TextEditor TextEditor) {
		...
		mb.add(m);
		m.add(mi = new JMenuItem(PCARDFrame.pc.intl.getText("Undo")));
		mi.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Z, s));
		mi.addActionListener(listener);
		undoMenu = mi;
		m.add(mi = new JMenuItem(PCARDFrame.pc.intl.getText("Redo")));
