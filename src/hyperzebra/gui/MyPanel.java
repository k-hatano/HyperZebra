...
public class MyPanel extends JPanel {
	...
	public MyPanel(PCARDFrame owner) {
		super();
		this.owner = owner;
	}
	...
	public int getBlendMode(String str) {
		if (str.equals("Copy"))
		...
		if (str.equals("Alpha Channel"))
			return blendMode.AlphaChannel;
	...
	protected void paintComponent(Graphics g) {
		...
		if (owner.tool != null) {
			...
			if (owner.tool != null && owner.redoBuf != null && owner.tool instanceof toolSelectInterface
					&& ((toolSelectInterface) owner.tool).isMove()) {
				...
				if (owner.blendMode != blendMode.Copy || owner.blendLevel != 100) {
					...
					BufferedImage newImage = new BufferedImage(flowImage.getWidth(), flowImage.getHeight(),
					...
					if (owner.blendMode == blendMode.AlphaChannel) {
						BufferedImage sfcImage = new BufferedImage(owner.getSurface().getWidth(),
								owner.getSurface().getHeight(), BufferedImage.TYPE_INT_ARGB);
						...
						makeBlendImage(newImage, flowImage, owner.getSurface(), offset, owner.blendMode,
								owner.blendLevel);
					}
				...
				if (owner.tool.getClass() == SelectTool.class) {
					...
					if (tl.move) {
						...
						mainPaneDraw(g, flowImage, tl.movePoint.x, tl.movePoint.y, owner.redoBuf.getWidth(),
								owner.redoBuf.getHeight());
					}
				} else if (owner.tool.getClass() == SmartSelectTool.class) {
		 ...
		 * &&PCARD.pc.tool==null){ return; }
		 */
	}

	public static void bordersDraw(Graphics g, int bit, int width, int height) {
		if (!PCARDFrame.useGrid)
			return;
	...
	void mainPaneDraw(Graphics g, BufferedImage img) {
		if (owner.bit > 1) {
			// 拡大表示
			g.drawImage(img, ((int) owner.bitLeft) * (-owner.bit), ((int) owner.bitTop) * (-owner.bit),
					((int) owner.bitLeft) * (-owner.bit) + owner.bgImg.getWidth() * owner.bit,
					((int) owner.bitTop) * (-owner.bit) + owner.bgImg.getHeight() * owner.bit, 0, 0,
					owner.bgImg.getWidth(), owner.bgImg.getHeight(), null);
		} else {
	...
	public void mainPaneDraw(Graphics g, BufferedImage img, int x, int y, int width, int height) {
		if (owner.bit > 1) {
			...
			g.drawImage(img, x * owner.bit - (int) owner.bitLeft * owner.bit,
					...
					y * owner.bit - (int) owner.bitTop * owner.bit + height * owner.bit, 0, 0, width, height, null);
		} else {
	...
	public static BufferedImage makeBlendImage(BufferedImage flowImage) {
		...
		makeBlendImage(newImage, flowImage, PaintTool.owner.getSurface(), offset, PaintTool.owner.blendMode,
				PaintTool.owner.blendLevel);

		return newImage;
	}

	private static void makeBlendImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int mode, int level) {
		if (flowImage.hasTileWriters()) {

			BufferedImage flowImage2 = new BufferedImage(flowImage.getWidth(), flowImage.getHeight(),
					BufferedImage.TYPE_INT_ARGB);
	...
	private static void copyImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// copyモードは色を変えるだけ
	...
	private static void blendImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
	...
	private static void addImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// addモードは黒を起点にした足し算
	...
	private static void subtractImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
	...
	private static void multiplyImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
	...
	private static void screenImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
	...
	private static void darkenImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// darkenモードは暗いほうを残す
	...
	private static void lightenImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// lightenモードは明るいほうを残す
	...
	private static void differenceImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// differenceモードは差の絶対値
	...
	private static void hueImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// hueモードは色相のみ
	...
	private static void colorImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// colorモードは色相と彩度
	...
	private static void saturationImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// saturationモードは彩度
	...
	private static void luminosityImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
		// luminosityモードは明度
	...
	private static void alphaImage(BufferedImage newImage, BufferedImage flowImage, BufferedImage surfaceImage,
			Point offset, int level) {
