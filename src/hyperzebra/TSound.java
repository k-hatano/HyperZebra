...
public class TSound implements LineListener {
	...
	public boolean Play(OStack stack, String soundRsrc, String[] neiro, int tempo, int vol) throws xTalkException {
		if (!use)
			return true;
		...
		// 長さ計算(秒)
		double length = 0.0;
		if (neiro[4].equalsIgnoreCase("w"))
			length = 240.0 / tempo;
		else if (neiro[4].equalsIgnoreCase("h"))
			length = 120.0 / tempo;
		else if (neiro[4].equalsIgnoreCase("q"))
			length = 60.0 / tempo;
		else if (neiro[4].equalsIgnoreCase("e"))
			length = 30.0 / tempo;
		else if (neiro[4].equalsIgnoreCase("s"))
			length = 15.0 / tempo;
		else if (neiro[4].equalsIgnoreCase("t"))
			length = 7.5 / tempo;
		else if (neiro[4].equalsIgnoreCase("x"))
			length = 3.75 / tempo;
		else if (!neiro[1].equals(""))
			length = 60.0 / tempo;
		if (neiro[5].equals("...."))
			length *= 1.5 * 1.5 * 1.5 * 1.5;
		if (neiro[5].equals("..."))
			length *= 1.5 * 1.5 * 1.5;
		if (neiro[5].equals(".."))
			length *= 1.5 * 1.5;// 1.75倍じゃないようだ。
		if (neiro[5].equals("."))
			length *= 1.5;
		...
		try {
			...
			sound = AudioSystem
					.getAudioInputStream(new File(path/* stack.file.getParent()+File.separatorChar+fileName */));
			...
			/*
			 * String path =
			 * (PCARDFrame.pc.stack.file.getParent()+File.separatorChar+"resource"+File.
			 * separatorChar+soundRsrc); try { sound = AudioSystem.getAudioInputStream(new
			 * File(path+".wav")); } catch (Exception e2) { try { sound =
			 * AudioSystem.getAudioInputStream(new File(path+".aiff")); } catch (Exception
			 * e3) {
			 */
		...
		try {
			...
			if (neiro[1].equals("") && length == 0.0) {
				...
				int code = 0;
				if (neiro[1].equalsIgnoreCase("c"))
					code = 0;
				else if (neiro[1].equalsIgnoreCase("d"))
					code = 2;
				else if (neiro[1].equalsIgnoreCase("e"))
					code = 4;
				else if (neiro[1].equalsIgnoreCase("f"))
					code = 5;
				else if (neiro[1].equalsIgnoreCase("g"))
					code = 7;
				else if (neiro[1].equalsIgnoreCase("a"))
					code = 9;
				else if (neiro[1].equalsIgnoreCase("b"))
					code = 11;
				if (neiro[2].equals("#"))
					code += 1;
				if (neiro[2].equals("b"))
				...
				if (neiro[3].equals(""))
					...
					code += 12 * Integer.valueOf(neiro[3]);
				...
				if (newSampRate < 8000)
					newSampRate = 8000;
				if (newSampRate > 48000)
					newSampRate = 48000;
				if (newFrameRate < 8000)
					newFrameRate = 8000;
				if (newFrameRate > 48000)
					newFrameRate = 48000;
				int newLength = fileFormat.getFrameSize()
						* (int) (voiceData.length / fileFormat.getFrameSize() * ((float) newSampRate / newRate));
				...
				if (newRate == newSampRate && nLength == newLength) {
					...
					if (!neiro[1].equalsIgnoreCase("r")) {
						for (int i = 0; i < newBuffer.length / fileFormat.getFrameSize(); i++) {
							if (((int) (i * ((float) newRate / newSampRate)) * fileFormat.getFrameSize())
									+ fileFormat.getFrameSize() >= voiceData.length)
								break;
							for (int j = 0; j < fileFormat.getFrameSize(); j++) {
								newBuffer[i * fileFormat.getFrameSize()
										+ j] = voiceData[((int) (i * ((float) newRate / newSampRate))
												* fileFormat.getFrameSize()) + j];
							}
						}
					}
					DataLine.Info di = new DataLine.Info(Clip.class, format);
